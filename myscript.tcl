...
set angular_speed 0.26
set vertical_speed 0.5
set speed 1
#TRACE INFO
...
proc createAUVWaypoints { } {
	...
	set curr_lat [ $position_(34) getLatitude_]
	set curr_lon [ $position_(34) getLongitude_]

	set toa [$position_($auv_id) addWayPoint $curr_lat $curr_lon $curr_depth $speed 0.0 ]
	puts "waypoint 3 lat = $curr_lat ; lon = $curr_lon ; depth = $curr_depth; toa = $toa"

	set curr_lat [ $position_(33) getLatitude_]
	set curr_lon [ $position_(33) getLongitude_]
...
proc rad2deg { radiant } {
	set degree [ expr ($radiant*180)/3,14159 ]
	return $degree
}


proc deg2rad { degree } {
	set radiant [ expr ($degree*3.14159)/180.0 ]
	return $radiant
}


proc lla2ecef { longitude latitude altitude } {
	set A [ expr 6378137.0 ]
	set B [ expr 6356752.3142 ]
	set long [ deg2rad $longitude ]
	set lat [ deg2rad $latitude ]
	set N [ expr ($A*$A)/sqrt(($A*$A)*(cos($long)*cos($long))+($B*$B)*(sin($long)*sin($long))) ]
	set x [ expr ($N + $altitude)*cos($long)*cos($lat) ]
	set y [ expr ($N + $altitude)*cos($long)*sin($lat) ]
	set z [ expr ((($B*$B)/($A*$A))*$N+$altitude)*sin($long) ]
	set coords(0) [ expr $x ]
	set coords(1) [ expr $y ]
	set coords(2) [ expr $z ]
...
proc ecef2lla { x y z } {
	set A [ expr 6378137.0 ]
        set B [ expr 6356752.3142 ]
	set E [ expr ($A*$A) - ($B*$B)/($A*$A) ]
	set F [ expr ($A*$A) - ($B*$B)/($B*$B) ]
	set P [ expr sqrt(($x*$x)+($y*$y)) ]
	set T [ expr atan(($z*$A)/($P*$B)) ]
	set coords(0) [ expr atan(($z+$F*$B*(sin($T)*sin($T)*sin($T)))/($P-$E*$A*cos($T)*cos($T)*cos($T))) ]
	set coords(1) [ expr atan($y/$x) ]
	set N [ expr (($A*$A)/sqrt($A*$A*cos($coords(0))*cos($coords(0))+$B*$B*sin($coords(0))*sin($coords(0)))) ]
	set coords(2) [ expr $P / cos($coords(0)) - $N ]
	set coords(1) [ rad2deg $coords(1) ]
	set coords(0) [ rad2deg $coords(0) ]
	return [ array get coords ]
}	
...
proc createAUVTrack {} {
	...
	for {set i 0} {$i < $length} {incr i} {
		...
		#salvo le coordinate geografiche
		set x_true [ expr $x ]
		set y_true [ expr $y ]
		set z_true [ expr $z ]
		...
		#salvo le coordinate geografiche
		set x_prev_true [ expr $x_prev ]
		set y_prev_true [ expr $y_prev ]
		set z_prev_true [ expr $z_prev ]
		#converto le coordinate geografiche in coordinate cartesiane
		array set coords [ lla2ecef $x_prev $y_prev $z_prev ]
		set x_prev [ expr $coords(0) ]
		set y_prev [ expr $coords(1) ]
		set z_prev [ expr $coords(2) ]
		...
		#salvo le coordinate geografiche
		set x_next_true [ expr $x_next ]
		set y_next_true [ expr $y_next ]
		set z_next_true [ expr $z_next ]
		#converto le coordinate geografiche in coordinate cartesiane
		array set  coords [ lla2ecef $x_next $y_next $z_next ]
		set x_next [ expr $coords(0) ]
		set y_next [ expr $coords(1) ]
		set z_next [ expr $coords(2) ]
		...
		if { $z_prev != $z && $i != 0 } {
			...
			set x_int [ expr $x_prev_true ]
			set y_int [ expr $y_prev_true ]
			set z_int [ expr $z_true ]
		...
		if { $i == $length -1 } {
			set toa [ $position_($auv_id) addLoopPoint $x_true $y_true $z_true $speed [ expr $angle/$angular_speed] 0 100 ]
			puts "debug: Aggiunto waypoint finale $x $y $z "
		} elseif { $i == 0 } {
			set toa [ $position_($auv_id) addStartWayPoint $x_true $y_true $z_true $speed [ expr $angle/$angular_speed ] ]  
			puts "debug: Aggiunto waypoint iniziale $x $y $z"
		} else {
			set toa [ $position_($auv_id) addWayPoint $x_true $y_true $z_true $speed [ expr $angle/$angular_speed ] ] 
